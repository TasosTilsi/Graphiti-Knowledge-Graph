...
must_haves:
  truths:
    - "A HUMAN-VERIFICATION.md exists in the Phase 06 directory with numbered, runnable test cases"
    ...
    - "The guide instructs how to update 06-VERIFICATION.md status after passing"
  artifacts:
    - path: ".planning/phases/06-automatic-capture/06-HUMAN-VERIFICATION.md"
      provides: "Step-by-step human verification guide for automatic capture runtime behavior"
      min_lines: 130
  key_links:
    - from: "06-HUMAN-VERIFICATION.md"
      to: ".planning/phases/06-automatic-capture/06-VERIFICATION.md"
      via: "update instructions at end of guide"
      pattern: "update.*status.*passed"
---

<objective>
Write the human verification guide for Phase 06 (Automatic Capture).

Phase 06 has `status: human_needed` in its VERIFICATION.md. The static verifier confirmed 7/7
observable truths through code analysis, but runtime behavior (git hook timing under 100ms,
perceivable conversation lag, end-to-end excluded file flow, captured knowledge queryability)
requires a human with the live environment.

This plan writes `.planning/phases/06-automatic-capture/06-HUMAN-VERIFICATION.md` — a
self-contained step-by-step guide a human can follow to confirm R4.1 and R4.2 in a live
environment. After the human completes all tests, they update the VERIFICATION.md status from
`human_needed` to `passed`.

Purpose: Close the verification gap for R4.1 (conversation-based capture) and R4.2 (git
post-commit hook) by providing a runnable checklist with copy-pasteable commands and explicit
pass/fail criteria.

Output: `.planning/phases/06-automatic-capture/06-HUMAN-VERIFICATION.md`
</objective>
...
<task type="auto">
  <name>Task 1: Write Phase 06 HUMAN-VERIFICATION.md</name>
  <files>.planning/phases/06-automatic-capture/06-HUMAN-VERIFICATION.md</files>
  <action>
Write the file `.planning/phases/06-automatic-capture/06-HUMAN-VERIFICATION.md` with the
...
grep -c "Test [0-9]:" .planning/phases/06-automatic-capture/06-HUMAN-VERIFICATION.md
grep "Pass criteria" .planning/phases/06-automatic-capture/06-HUMAN-VERIFICATION.md | wc -l
grep "Update VERIFICATION.md" .planning/phases/06-automatic-capture/06-HUMAN-VERIFICATION.md
```

Expected: file exists, at least 5 "Test N:" occurrences, at least 4 "Pass criteria" lines,
"Update VERIFICATION.md" section present.
  </verify>
  <done>
`.planning/phases/06-automatic-capture/06-HUMAN-VERIFICATION.md` exists with:
- 5 numbered test cases (Test 1 through Test 5)
...
- Instructions to update 06-VERIFICATION.md status after passing
  </done>
</task>
...
- `.planning/phases/06-automatic-capture/06-HUMAN-VERIFICATION.md` exists on disk
- File contains 5 test cases with runnable commands
- File contains "After Passing: Update VERIFICATION.md" section
- File contains a Troubleshooting section
- A human following the guide can confirm R4.1, R4.2 without reading source code
</verification>

<success_criteria>
1. Phase 06 HUMAN-VERIFICATION.md exists with numbered, runnable test cases
...
4. Troubleshooting section covers common failure modes (hook not executable, LLM unavailable)
5. Final section instructs how to update 06-VERIFICATION.md from human_needed to passed
</success_criteria>

<output>
After completion, create `.planning/phases/8.5-gap-closure-human-runtime-verification-inserted/8.5-02-SUMMARY.md`
</output>
